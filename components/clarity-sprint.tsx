...
function generateMarkdown(answers: Record<string, string>, aiInsight?: string, progress?: number): string {
  const sections = QUESTIONS.map((q) => {
    const answer = answers[q.id]?.trim() || "_Not answered_"
    return `## ${q.section}\n${answer}`
  })

  const stage = progress !== undefined ? getClarityStage(progress) : null
  const clarityStatus = stage ? `## Clarity Status\n${stage.label} (${progress}% complete)` : ""
  const insightSection = aiInsight ? `## Insight\n${aiInsight}` : ""
  const footer = `---\nGenerated by Clarity — a tool by Enter404 for designers navigating the AI era.\nhttps://enter404.com`

  return [`# Project Scope`, ...sections, clarityStatus, insightSection, footer]
    .filter(Boolean)
    .join("\n\n")
}
...
export function ClaritySprint() {
  ...
  async function handleCopy() {
    try {
      await navigator.clipboard.writeText(generateMarkdown(answers, aiInsight, progress))
      setCopied(true)
  ...
  function handleDownload() {
    const blob = new Blob([generateMarkdown(answers, aiInsight, progress)], { type: "text/markdown" })
